...
function AuthPage({ defaultMode = 'login' }) {
  ...
  return <main className="auth-page">
    <section className="brand-panel" aria-label="CleanMetric introduction"><div className="brand"><span className="brand-mark" aria-hidden="true"><i /><i /><i /></span><span>CleanMetric</span></div><div className="brand-copy"><p className="eyebrow">ANALYZE WITH CLARITY</p><h1>Make every metric<br />matter.</h1><p>Bring your data into focus and turn the numbers that matter into confident decisions.</p></div><div className="metric-card" aria-hidden="true"><div className="metric-card-top"><span>Performance overview</span><span className="live-dot">Live</span></div><strong>84.6<span>%</span></strong><div className="chart">{[28, 40, 33, 57, 49, 72, 65, 92].map((height) => <span key={height} style={{ height: `${height}%` }} />)}</div></div><p className="panel-footer">© 2026 CleanMetric. All rights reserved.</p></section>
    <section className="form-panel"><div className="mobile-brand brand"><span className="brand-mark" aria-hidden="true"><i /><i /><i /></span><span>CleanMetric</span></div><div className="auth-card">
      ...
      <div className="form-heading"><p className="eyebrow">{isLogin ? 'WELCOME BACK' : 'GET STARTED'}</p><h2>{isLogin ? 'Sign in to CleanMetric' : 'Create your account'}</h2><p>{isLogin ? 'Enter your details to access your workspace.' : 'Start turning your metrics into meaningful insight.'}</p></div>
